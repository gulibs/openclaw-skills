...
- **`steps`**：对应 manifest 中非单场步（00–03、05–06）的摘要；`step_id` 建议与文件名一致（如 `04d_game_7341_conclusion` 或赛后 `04f_game_7341_review`；单场长文也可只写在 `games`）。
...
5. **04** — 每个 `game_id` 依 `game_phase`：**赛前**：`04a_*_facts` → `04b_*_form` → `04c_*_features` → `04d_*_conclusion`。**赛后（已结束）**：`04a`（盲事实，无比分）→ `04b` → `04c` → `04d_*_analysis`（先验）→ `04e_*_truth`（真实赛果）→ `04f_*_review`（对照复盘）。04a–04c **禁止**写出最终信心分与投注式推荐；**禁止**省略任一场或任一轮。
...
- **`pick_zh` / `summary_zh` / `report_markdown_zh` 中的推荐句**：**禁止**仅用 `game_id` 指代场次（如「7348 大小分 + 7349 胜负」）。须写清 **队名 + 玩法 + 方向**，并与 `bet_type_hint` 一致：
  ...
  - **让分**：`**让分主胜**` 或 `**让分主负**`，并配合 `（客）{客} vs {主}（±线）`（见下条）。
- **`bet_type_hint`（让分）**：须为 **`rfsf_home`** 或 **`rfsf_away`**，分别对应体彩 **让分主胜（主队赢盘）** / **让分主负（客队赢盘）**；**勿**只写 **`rfsf`**，否则管理端卡片无法稳定展示「推荐注项」。
- **让分 `pick_zh`**：须用 **（客）{客队} vs {主队}（±线）** 写清推荐方向（线与数据包 `rfsf_home.line` 一致：负 = 主队让，正 = 主队受让）；**勿**使用「某队让分（线）」单句。正文侧重理由，**勿**再写一套与数据包不一致的盘口线（避免与入库 `rfsf_board_zh` 冲突）。让分主胜 = 主队赢盘（分差 + 线 > 0），让分主负 = 客队赢盘。
