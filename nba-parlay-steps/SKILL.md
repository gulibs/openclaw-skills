...
- **目的**：用 **分步** 控制上下文体积、让 **01_rules** 与赛程/盘口索引先被读入，避免单次对话塞爆；同时让 **产出可审计**（`output/` 与 `manifest` 对齐）。
...
- **`steps`**：对应 manifest 中非单场步（00–03、05–06）的摘要；`step_id` 建议与文件名一致（如 `04d0_game_7341_espn_verify`、`04d_game_7341_conclusion` 或赛后 `04f_game_7341_review`；单场长文也可只写在 `games`）。
...
- 提到 **串关**、**parlay_recommendation.json**、**manifest.json**、**04a/04d（赛前）或 04f（赛后）**、**OpenClaw 串关**、**体彩分步分析**
...
## 建议执行顺序（按 manifest 的 `order`）

按 `manifest.steps` 中 **`order` 升序** 对应文件执行（与文件名 `00_`、`01_`…`06_` 一致）。**务必先完成 01**，再跑 02/03，否则易误解 `level_hint` 与 Fail Fast。

1. **00** `00_README.md` — 理解分步说明与磁盘输出约定。
...
5. **04** — 每个 `game_id` 依 `game_phase`：**赛前**：`04a_*_facts` → `04b_*_form` → `04c_*_features` → **`04d0_*_espn_verify`（ESPN 公开页交叉验证，补全/核对伤停与深度图等）** → `04d_*_conclusion`。**赛后（已结束）**：`04a`（盲事实，无比分）→ `04b` → `04c` → **`04d0_*_espn_verify`** → `04d_*_analysis`（先验）→ `04e_*_truth`（真实赛果）→ `04f_*_review`（对照复盘）。04a–04c **禁止**写出最终信心分与投注式推荐；**禁止**省略任一场或任一轮。04d0 **不**替代数据包内的体彩盘口事实，仅作第三方公开信息 sanity check；主仓库文档见 `docs/ESPN_NBA_TEAM_IMPORTANT_PAGES.md`（`abbr`/`slug` 与重要子页 URL）。
6. **05** `05_synthesis_parlay.md` — 串关或赛后复盘合成（腿须与入库赔率一致）。**仅当** scope 内每场 **赛前 04d** 或 **赛后 04f** 已完成后再做（会话无历史时可请用户粘贴各场摘要，但不得伪造未做场次）。
...
- **做法**：按 **`01_rules.md` / `05_synthesis_parlay.md`** 与数据包内各场 **赛前 04d** 或 **赛后 04f** 的结论，在 **组合可行** 的前提下，给出 **多档串关**（赛前）或 **复盘**（赛后），而不是默认只写一档 2 串 1。
...
- **组合可行性（必须先判再写）**：未通过则 **不要**把该组合写入 `parlays`，或单独说明不可行原因。
  - 所选场次均属同一 `beijing_game_date` 分析范围，且与 `02_schedule.md` 中可串关场次一致；若步骤 2 已判定**无法对齐场次**，则不再强行组关。
...
- 数据包**不包含**内部 `model_predictions` / `edge_results`（ensemble 胜率、EV 等）。输出中**勿**用「**模型预测**」指代上述未提供内容。
...
**写盘前自检（须能在心中或回复中核对）**：`manifest.game_ids_scope` 内每个 `game_id` 是否都已完成对应 **`04d_game_<id>_conclusion`（赛前）或 `04f_game_<id>_review`（赛后）**（或兼容旧名 `04_game_<id>`）；**是** → 才可进入 05/06；**仅当** 全部步骤完成且与 scope 一致时，才把 `meta.json` 的 `status` 设为 `completed`，并把 `parlay_recommendation.json` 的 `analysis_complete` 设为 `true`。未完成则保持 `in_progress` 或 `failed`，**勿**提前同步终稿。
...
```json
{
  ...
  "finished_steps": ["00_README", "01_rules", "02_schedule", "03_odds_index", "04d_game_7341_conclusion", "04f_game_7342_review", "05_synthesis", "06_output"],
...
- **`finished_steps`**：须**逐项列出**每场完成标记，如赛前 `04d_game_7341_conclusion`、赛后 `04f_game_7342_review`（或兼容 `04_game_<id>`），勿仅用占位符。
...
- 每条 leg **必须**含 **`matchup_zh`**（推荐）或并列 **`home_team`** / **`away_team`**，与步骤 2 赛程表一致；**勿**只写玩法与盘口线而不写对阵。后端同步入库时会按 `game_id` 从库表补全队名，并尽量补 **`spread_line` / `rfsf_board_zh`（（客）客 vs 主（±线））**，但 **`pick_zh` 仍须**按体彩习惯自写让分句。
- **`pick_zh` / `summary_zh` / `report_markdown_zh` 中的推荐句**：**禁止**仅用 `game_id` 指代场次（如「7348 大小分 + 7349 胜负」）。须写清 **队名 + 玩法 + 方向**，并与 `bet_type_hint` 一致：
  - **大小分**：`{客} vs {主}，总分线 x.x：**大分** / **小分**`（对应 `dxf_over` / `dxf_under`）。
  - **胜负**：`**主胜** / **客胜**` 或 **`{队名} 胜**`。
  - **胜分差**：`**主胜/客胜** + **分差档位**`（与体彩 12 档命名一致）；缺入库则不要写该腿。
  - **让分**：`**让分主胜**` 或 `**让分主负**`，并配合 **客队「`away_team`」vs 主队「`home_team`」（±线）**（数据包每场 JSON 含 `away_team` / `home_team`，**勿**用「（客）队名」紧挨队名，易把主客写反）。
- **`bet_type_hint`（让分）**：须为 **`rfsf_home`** 或 **`rfsf_away`**，分别对应体彩 **让分主胜（主队赢盘）** / **让分主负（客队赢盘）**；**勿**只写 **`rfsf`**，否则管理端卡片无法稳定展示「推荐注项」。
- **让分 `pick_zh`**：须用 **客队「`away_team`」vs 主队「`home_team`」（±线）** 写清推荐方向；**线号 = `rfsf_home.line`（主队视角）**：**正 = 主队加分/受让**（口语可称客队让主队），**负 = 主队减分/让分**。**禁止**把「+17.5」说成「主队让 17.5 分」。每场若有 **`rfsf_home_line_semantics_zh`**，须与之一致判盘：**让分主胜** ⟺ `margin + line > 0`（`margin` = 主得分 − 客得分）。风险句中「客场/主场」须与 `away_team`/`home_team` 一致；**勿**用「主队大胜才赢盘」类美式叙事套到 **正线（主队受让）** 上。**勿**使用「某队让分（线）」单句。正文侧重理由，**勿**再写一套与数据包不一致的盘口线（避免与入库 `rfsf_board_zh` 冲突）。
